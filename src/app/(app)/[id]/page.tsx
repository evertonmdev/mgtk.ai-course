			...
			$("*").each((i, el) => {
				...
				switch (tag) {
					...
					case "pre":
						...
						renders.push(
							<pre
								key={i}
								// biome-ignore lint/security/noDangerouslySetInnerHtml: <explanation>
								dangerouslySetInnerHTML={{ __html: content?.trim() as string }}
							/>,
						);
						break;
					case "codigoxx": {
					...
					case "ul": {
						...
						$(el)
							.find("li")
							.map((_i, el) => list_contents.push($(el).html() as string));
						renders.push(
							<ul key={i}>
					...
					case "h6":
						...
						renders.push(
							React.createElement(
								tag,
								{ key: i, className: "text-red-500 dark:text-violet-600" },
								text,
							),
						);
						break;
