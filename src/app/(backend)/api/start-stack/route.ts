...
async function startStack({ id }: { id: string }) {
	...
	try {
		const steps = (await googleGetSteps({
			thema: stack.tema,
			observations: stack.observacao,
		})) as string[];

		await updateStackStatus({
			id: stack.id,
		...
		for (const etapa in etapas) {
			...
			if (parseInt(etapa) === etapas.length - 1) {
				await googleGenerateInfos({ stack_id: stack.id })
					.then(async (response) => {
...
async function createContent({
	...
	try {
		...
		await updateStackStatus({
			id: stack_id,
			status: `Criando conteúdo da etapa ${nome_etapa}`,
		});
		for (let attempt = 0; attempt < 5; attempt++) {
			if (attempt > 0) {
				...
				console.log(
					chalk.yellow(
						`Tentativa ${
							attempt + 1
						} de criar conteúdo da etapa ${nome_etapa}, aumentando temperatura para ${chalk.green(
							temperature,
						)}`,
					),
				);
			...
			try {
				...
				console.error(
					"Falha ao atualizar conteúdo da etapa",
					nome_etapa,
					error.message,
				);
			}
		...
		throw new Error(
			`Falha ao gerar conteúdo da etapa ${nome_etapa} após 5 tentativas`,
		);
		// biome-ignore lint/suspicious/noExplicitAny: <explanation>
	} catch (error: any) {
		throw new Error(
			`Falha ao criar conteúdo da etapa ${nome_etapa}: ${error.message}`,
		);
	}
}
