...
const FormCard: React.FunctionComponent = () => {
	...
	const onSubmit = async (data: IFormCourseCreateData) => {
		try {
			...
			toast.info("ATENÇÃO!!!", {
				description:
					"Devido às restrições técnicas da plataforma, o processo de criação do curso pode demandar um pouco mais de tempo. Por favor, evite fechar a aba até que o curso esteja completamente pronto para garantir uma experiência sem contratempos!",
				duration: 15000,
			});
				...
				for (let i = 0; i < 500; i++) {
					...
					if (status?.status === "Falhou") {
						...
						toast.error("Lamento, ocorreu um erro ao criar o curso.", {
							description:
								"As causas para isso podem ser variadas, mas o mais comum é a falta de recursos para criar o curso ou conteudo nocivo.",
						});
	...
	return (
		<Card className="max-lg:w-full w-1/3 h-fit min-h-[400px] py-10">
			<CardHeader className="w-full justify-between items-center">
				<h3 className="font-bold text-xl text-foreground-600">
			...
			<CardBody>
				<form
					...
					className="flex h-fit flex-col justify-start items-center gap-3"
				>
					{!status.active ? (
						<>
							<Input
								...
								label="Tema do curso"
							/>
							<Input {...register("observacao")} label="Algum adicional?" />
						</>
					) : (
						<div className="w-[60%] h-fit min-h-[100px] flex justify-center items-center flex-col">
							<h2 className="text-lg font-semibold mb-5">{status.status}</h2>
							<Progress
								size="sm"
								value={
									status.status === "Iniciando"
										? 0
										: status.status === "Criando passo a passo..."
										  ? 20
										  : status.status === "Criando conteudo..."
											  ? 40
